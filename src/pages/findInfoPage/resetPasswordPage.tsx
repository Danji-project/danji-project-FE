...
const PasswordForm = () =>
{
  ...
  const [password, setPassword] = useState<PasswordInfo>(new PasswordInfo('', false));
  const [passwordError, setPasswordError] = useState<string|null>(null);

  const [passwordConfirm, setPaasswordConfirm] = useState<PasswordInfo>(new PasswordInfo('', false));
  ...
  const checkPasswordConfirm = () => {
      ...
      let check =validateCheck("PASSWORD_CONFIRM_CHECK", passwordConfirm.data, password.data)!;
      ...
      mutationFn: async () => {
        try {
          ...
          const response = await axios.post(
            `${import.meta.env.VITE_API_URL}${API_ENDPOINTS.FIND.RESET_PW}`,
  ...
  return(
      ...
      { email?
        <div className={`${styles['content-div']}`} onLoad={()=>{localStorage.removeItem('eamilcash');}}>
          ...
          <div style={{paddingBottom:"20px"}}>
            <InputFiled label="새 비밀번호"
                        placeholder="영문,숫자,특수문자 포함 8~16자"
                        type="password"
                        name="password"
                        showPasswordToggle
                        value={password?.data}
                        onChange={(e) => {setPassword(prev => prev.setData(e.target.value));}}
          ...
          <div style={{paddingBottom:"20px"}}>
            <InputFiled label="비밀번호 확인"
                        placeholder="영문,숫자,특수문자 포함 8~16자"
                        type="password"
                        name="password"
                        showPasswordToggle
                        value={passwordConfirm?.data}
                        onChange={(e) => {setPaasswordConfirm(prev => prev.setData(e.target.value));}}
          ...
          <button className={`${styles["nomal-button"]} ${styles["nomal-button--valid"]} ${styles["submit-button"]}`}
        ...
        <div className={`${styles['content-div']}`}>
          ...
          <button className={`${styles["nomal-button"]} ${styles["nomal-button--valid"]} ${styles["submit-button"]}`}
                  onClick={() => {navigate("/find", { replace: true }); localStorage.setItem('clickpw', 'true');}}> 
                  이전 페이지로 돌아가기</button>
        </div>
      }
      </>
