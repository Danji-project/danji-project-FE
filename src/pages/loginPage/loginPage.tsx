...
const LoginForm = () => {
  ...
  const checkEmail = () => {
    ...
    else{setEmailError('이메일 형식이 올바르지 않습니다. 예: example@domain.com');}
  }
      ...
      <div>
        <form onSubmit={handleSubmit}>
            ...
            <div style={{textAlign:"center", marginTop:"40px"}}>
              <button className={`${styles['login-form__submit-button']} ${isValidInputs ? styles['login-form__submit-button--valid'] : ''}`} 
                      onMouseOver={() => {checkEmail();}}
                      type="submit" disabled={isValidInputs? false : true}>로그인</button>
              <p className={`${styles['login-gray-small-text']}`}>아직 회원이 아니신가요? <a className={`${styles['login-blue-small-text']}`} href=''>회원가입</a></p>
            </div>
        </form>
      </div>
      </>
...
const SocialLogin = () => {
  return (
    <div style={{flexDirection:"row", margin:"0 auto", textAlign:"center"}}>
      <a href='https://danjitalk.duckdns.org/oauth2/authorization/kakao'><img src={KakaoIcon} /></a>
      <a href='https://danjitalk.duckdns.org/oauth2/authorization/google'><img style={{margin:"0px 20px"}} src={GoogleIcon}/></a>
      <a href='https://danjitalk.duckdns.org/oauth2/authorization/naver'><img src={NaverIcon} /></a>
    </div>
  );
}
